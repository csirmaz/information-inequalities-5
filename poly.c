...
static int closetoint(double x, long scale, long *res)
/* check if x is close to an integer */
{double y;
//   x  (-2.5,-1.5]  (-1.5,-0.5) (-0.5,0.5) [0.5,1.5) [1.5,2.5)
//*res      -2           -1           0         1         2
    *res=intfloor(x); y=x-*res;
    if(scale>5l) y *= 2.0/(double)scale;
    return (y<SCALE_EPS && y>-SCALE_EPS);
}

static long denum(double x,long scale,long *res)
/* return d such that d*x is close to an integer */
{int i;
    for(i=1;i<1000;i++)if(closetoint(i*x,i*scale,res)) return (long)i;
    return 1l;
}

/* long gcd(a,b); long lcm(a,b)
*     compute gcd and lcm of two integers */
static long gcd(long a,long b) // a,b>=0
{   if(a==0 || a==b) return b;
    if(b==0) return a;
    if(a<b) return gcd(b%a,a);
    return gcd(a%b,b);
}
static long lcm(long a,long b) // a,b>=1
{   if(b==1 || a==b) return a;
    if(a==1) return b;
    return a*(b/gcd(a,b));
}

static char *formatvalue(long d,double v)
/* formatted output of a vertex coordinate */
{static char buf[80]; long en,den,cc;
    round_to(&v); // make it close to a rational, if possible
    if(PARAMS(VertexAsFraction)){
        if(closetoint(v*(double)d,d,&en)){
            den=d;
        } else if((den=denum(v,1,&en))>1){
            ;
        } else if((den=denum(v*(double)d,d,&en))>1){
            den*=d;
        } else {
            sprintf(buf,"%.14g",v);
            return buf;
        }
        // v = en/den, make it as simple as possible
        cc=gcd(en<0 ? -en : en, den);
        en /= cc; den /= cc;
        if(den==1l){sprintf(buf,"%ld",en); return buf; }
        sprintf(buf,"%ld/%ld",en,den); return buf;
    }
    sprintf(buf,"%.14g",v);
    return buf;
}
...
{int j; long d,dummy; double w,dir; const double *coord/*0:DIM*/;
    coord=VertexCoords(vno);
    dir = PARAMS(Direction) ? -1.0 : 1.0;

    d=1l;for(j=0;d<1000l && j<=DIM;j++){
       w=d*coord[j]; round_to(&w); d=lcm(d,denum(w,1,&dummy));
    }
    report(channel,"%c ",is_finalVertex(vno) ? 'V' : 'v');
    for(j=0;j<=DIM;j++){
       report(channel," %s",formatvalue(d,dir*coord[j]));
    }
    report(channel,"\n");
}
...
{int j; long d,dummy; double w,dir;
    dir = PARAMS(Direction) ? -1.0 : 1.0;
    d=1l;for(j=0;d<10000l && j<=DIM;j++){ 
        w=coord[j];round_to(&w); d=lcm(d,denum(w,1l,&dummy));
    }
    for(j=0;j<=DIM;j++){
        w=d*coord[j]; if(j==DIM) w *= dir; round_to(&w);
        if(closetoint(w,d,&dummy)) w=dummy;
        report(channel," %.14lg",w);
    }
    report(channel,"\n");
}
